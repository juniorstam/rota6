...
export function RoutePlannerScreen() {
  ...
  function openNavigationApp(app: "google" | "waze") {
    ...
    if (app === "google") {
      ...
      // Waze só suporta origem → destino direto (sem múltiplas paradas via URL padrão)
      window.open(
  ...
  // Distância estimada por trecho: divide igualmente entre os trechos
  const legDistances = useMemo(() => {
    if (!preview?.distanceKm || legPoints.length < 2) return [];
    const totalLegs = legPoints.length - 1;
    const perLeg = preview.distanceKm / totalLegs;
    return Array.from({ length: totalLegs }, () => perLeg);
  ...
  return (
    <div className="fixed inset-0 z-[60] flex flex-col">
      ...
      <div className="pointer-events-none relative z-10 flex h-full flex-col justify-between p-3 pb-28 md:p-4 md:pb-6">
        ...
        <div className="pointer-events-auto w-full">
          <div className="overflow-hidden rounded-[22px] border border-white/15 bg-black/80 shadow-[0_8px_32px_rgba(0,0,0,0.5)] backdrop-blur-xl">
            ...
            <button
              ...
              aria-label={topCollapsed ? "Expandir painel" : "Recolher painel"}
            >
        ...
        {/* ══ SUGGESTIONS — carrossel com imagens (estilo Google Maps) ══ */}
        <div className="pointer-events-auto w-full">
                ...
                {routeSuggestions.map((place) => (
                  <article key={place.id} className="flex shrink-0 w-[160px] flex-col overflow-hidden rounded-[14px] border border-white/10 bg-white/10">
                    ...
                    <div className="flex flex-1 flex-col gap-1 p-2.5">
                      ...
                      <p className="text-[10px] text-white/50">{ROUTE_SUGGESTION_CATEGORY_LABELS[place.category]}</p>
                      <button
                        type="button"
          ...
          <div className="rounded-[22px] border border-white/15 bg-black/80 shadow-[0_-8px_32px_rgba(0,0,0,0.4)] backdrop-blur-xl">
            ...
            {!bottomCollapsed && (
              <div className="px-4 pb-4">
                ...
                {/* LEG SCROLL com distâncias entre pontos */}
                ...
                {!preview ? (
                  ...
                  <div className="flex flex-col gap-2.5">
                    {/* Iniciar navegação — abre modal de escolha */}
                    <button
                      type="button"
