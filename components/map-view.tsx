...
export function MapView({
  ...
  return (
    <section className="overflow-hidden rounded-[32px] border border-border bg-surface">
      <div className="relative min-h-[360px] bg-[radial-gradient(circle_at_top,rgba(249,115,22,0.12),transparent_30%),linear-gradient(180deg,#ffffff_0%,#f8fafc_100%)] p-6">
        <div className="absolute inset-0 opacity-40 [background-image:linear-gradient(rgba(148,163,184,0.16)_1px,transparent_1px),linear-gradient(90deg,rgba(148,163,184,0.16)_1px,transparent_1px)] [background-size:32px_32px]" />
        ...
        <div className="relative flex h-full flex-col justify-between gap-6">
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="rounded-full border border-border bg-surfaceAlt p-3 text-accent">
              <Navigation size={18} />
            </div>
          </div>

          <div className="relative flex-1 overflow-hidden rounded-[28px] border border-border bg-surfaceAlt/70 p-5">
              ...
              {polyline.length > 0 ? (
                polyline.map((point, index) => (
                  <div key={`${point.lat}-${point.lng}`} className="flex flex-col items-center gap-2">
                    ...
                    {index < polyline.length - 1 && <div className="h-16 w-px bg-border md:h-24" />}
                  </div>
                ))
              ) : (
